...
Seven checks: structure & naming · size budgets (lines + bytes) · dead links & ID references · rot detection (`verified:` date vs git activity on `code:` globs) · orphaned ledger items · inbox backlog · history-trace & reading-surface noise (truth docs turning into changelogs or audit reports; scans `truthDirs` for not-yet-migrated projects). **Red blocks settlement (exit 1); yellow reminds.**
